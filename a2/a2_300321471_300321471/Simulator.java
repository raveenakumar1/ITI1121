	...
	public void simulate() {
		...
		while (clock < steps) {
			...
			if(RandomGenerator.eventOccurred(probabilityOfArrivalPerSec)){
				Car cars = RandomGenerator.generateRandomCar();
				incomingQueue.enqueue(new Spot(cars,clock));
			}
			for(int i = 0; i < lot.getNumRows(); i++){
				for(int j = 0; j < lot.getNumSpotsPerRow(); j++){
					if(lot.getSpotAt(i,j) != null){
						int duration = clock - lot.getSpotAt(i,j).getTimestamp();
						if(duration == MAX_PARKING_DURATION){
							outgoingQueue.enqueue(lot.remove(i,j));
			...
			if(carFailed != null){
				lot.attemptParking(carFailed,clock);
				System.out.println(carFailed + " ENTERED at timestep " + clock + "; occupancy is at " + lot.getTotalOccupancy());
				carFailed = null;
			}else{
				if(!incomingQueue.isEmpty()) {
					Car carIn = incomingQueue.dequeue().getCar();
					if (lot.attemptParking(carIn, clock) == true) {
						System.out.println(carIn + " ENTERED at timestep " + clock + "; occupancy is at " + lot.getTotalOccupancy());
					} else {
						carFailed = carIn;
					}
				}
			...
			if(!outgoingQueue.isEmpty()){
				Spot carOut = outgoingQueue.dequeue();
				System.out.println(carOut.getCar() + " EXITED at timestep " + clock + "; occupancy is at " + lot.getTotalOccupancy());
			}
			clock++;
		}
	}
