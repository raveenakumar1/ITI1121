...
public class ParkingLot {
	...
	public boolean canParkAt(int i, int j, Car c) {
		...
		if (carType == CarType.ELECTRIC) {
			...
			return (spotType == CarType.ELECTRIC) || (spotType == CarType.SMALL) || (spotType == CarType.REGULAR) || (spotType == CarType.LARGE);
	...
	private void calculateLotDimensions(String strFilename) throws Exception {
		...
		while (scanner.hasNext()) {
			...
			if (str.isEmpty()) {
				...
				numSpotsPerRow = Math.max(tokens.length, numSpotsPerRow);
			}
		...
		while (scanner.hasNext()) {
			...
			if (str.isEmpty()){
				...
				if (tokens.length != 4 || !tokens[0].trim().matches("\\d+") || !tokens[1].trim().matches("\\d+")){
					System.out.println("Skipped line "+ lineCount + " because something went wrong");
					continue;
				}
